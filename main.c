...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        printf("Usage: %s <fichier.algo> [sortie.c]\n", argv[0]);
        return 1;
    }

    const char* chemin = argv[1];
    const char* sortie_c = (argc >= 3) ? argv[2] : "out.c";
    ...
    // 7) Génération du code C (fichier de sortie)
    if (!cgen_generate(prog, sortie_c)) {
        printf("\nGénération C échouée.\n");
        code_retour = 5;
        goto cleanup;
    }
    printf("\nCode C généré : %s\n", sortie_c);
